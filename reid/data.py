...
from reid.utils.RandomErasing import RandomErasing
from reid.utils.RandomSampler import RandomSampler
from reid.opt import opt
import os
import re
...
class Data():
    def __init__(self):
        ...
        if opt.mode == 'vis':
            self.query_image = test_transform(default_loader(os.path.join(self.queryset.data_path, opt.query_image)))

        if opt.mode == 'compare':
            self.compare_img_a = test_transform(default_loader(opt.compare_img_a))
            self.compare_img_b = test_transform(default_loader(opt.compare_img_b))

            self.query_image = test_transform(default_loader("cache/query.jpg"))
            self.compare_img_b = test_transform(default_loader(opt.compare_img_b))
